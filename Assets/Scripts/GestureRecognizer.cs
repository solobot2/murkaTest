...
public class GestureRecognizer:MonoBehaviour
{
	...
	public int lastMatchedNum;
	// recognizer settings
	public int maxPoints = 64;
	// max number of point in the gesture
	public int sizeOfScaleRect = 500;
	// the size of the bounding box
	public int compareDetail = 15;
	// Number of matching iterations (CPU consuming)
	public int angleRange = 45;
	// Angle detail level of when matching with templates

	public float minScoreToPass = 0.7f;

	public  int recordDone = 0;
	public string stringToEdit = "Enter a Template name";
	public ArrayList newTemplateArr;
    public float newTemplateAngle;
	...
	public bool startRecognizer(ArrayList pointArray)
	{
		...
		return gestureMatch(pointArray); 
	}
	...
	public void recordTemplate(ArrayList pointArray)
	{
	    ...
	    newTemplateAngle = Mathf.Rad2Deg*radians;
	...
	public ArrayList RotateGesture(ArrayList pointArray, float radians, Vector3 center)
	{
	...
	private  bool gestureMatch(ArrayList pointArray)
	{
		float tempDistance = Mathf.Infinity;
		int count = 0;

		for (int i = 0; i < GestureTemplates.Instance.Templates.Count; ++i)
		{
			float distance = calcDistanceAtOptimalAngle(pointArray, (ArrayList)GestureTemplates.Instance.Templates[i], -angleRange, angleRange);
...
/*		if (score < 0.7f)
		...
		*/
		...
		{
			Debug.Log("RESULT: " + GestureTemplates.Instance.TemplateNames[count] + " SCORE: " + score);
			lastMatchedNum = count;
			return true;
		}
	}
	...
	public  Vector2 calcCenterOfGesture(ArrayList pointArray)
	{
	...
	private float calcDistanceAtOptimalAngle(ArrayList pointArray, ArrayList T, float negativeAngle, float positiveAngle)
	{
	...
	private float calcDistanceAtAngle(ArrayList pointArray, ArrayList T, float radians)
	{
	...
	private float calcGestureTemplateDistance(ArrayList newRotatedPoints, ArrayList templatePoints)
	{
